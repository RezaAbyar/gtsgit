...
function setLoading(btn,status){
    ...
    btn.innerText = (!status ? btn.getAttribute("data-init-text") : "در حال بارگذاری ...");
}
...
function toast(msg,type="error") {
    ...
    document.querySelectorAll(".toast-description h3").item(0).innerText = type === "error" ? "خطا" : "موفق";
    ...
    if(formButton){
        setInterval(function(){
            if(username?.value && username.value.length === 11){
                ...
                $(".username-error").innerText = (validationStatus ? "" : "شماره موبایل وارد شده باید 11 رقمی باشد و با 09 شروع شود");
            }
        },500)
        ...
        username.addEventListener("keyup",function(event){
            ...
            $(".username-error").item(0).innerText = (validationStatus ? "" : "شماره موبایل وارد شده باید 11 رقمی باشد و با 09 شروع شود");
        });
...
function sendOtpService() {
    console.log("sendOtp Service Call");
    ...
    $("#otp-mobile-holder").innerText = (mobile);
        ...
        httpService(
            ...
            function (xhttp) {
                ...
                if(xhttp.status === 200){
                    ...
                    if(json.code === 400){
                        toast('لطفا اطلاعات وارد شده را بررسی نمایید')
                    }else{
                        toast(json?.enErrorMessage || "لطفا مجددا تلاش نمایید")
                    }
                }
            },
            "POST",
...
function submitEvent(event,parentSelector) {
    ...
    if(password?.length === 5){
        ...
        toast('لطفا کد فعال سازی را بصورت کامل وارد نمایید');
    }
    ...
    if($(".captcha")?.length){
        httpService("/client/v1/captcha", {}, headers, function (xhttp) {
            if (xhttp.status === 200) {
                ...
                toast("خطا در ریافت تصویر کد امنیتی");
            }
        });
    }
}
    ...
    if($(".captcha")?.length && $(".captcha_key")?.item(0)?.value){
        httpService("/client/v1/captcha/audio/"+$(".captcha_key")?.item(0)?.value, {}, headers, function (xhttp) {
            ...
            if (xhttp.status === 200) {
                ...
                toast("خطا در ریافت صوت کد امنیتی");
            }
        },"GET",true,true,true);
    }
}
        ...
        if (window.seconds < 0) {
            ...
            if(window.seconds <= 0){
                ...
                if (submitLoginBtn) {
                    ...
                    submitLoginBtn.innerHTML = `<img src="../statica/web/assets/images/SendIcon.svg" alt="" /> <span>ارسال مجدد کد فعال سازی</span>`;
...
function handleRegisteration(event){
    ...
    if(birthDateFieldExists){
        if (!birthDate?.value || !birthDate.value?.length || birthDate.value.length < 8) {
            errors++;
            toast('تاریخ تولد را وارد نمایید')
        }
    }
...
function handleRegister(event) {
    ...
    if (!postalCodeFieldExists && $("#description")) {
        $("#description").innerText = (`کاربر گرامی جهت احراز هویت تاریخ تولد و کد ملی خود راوارد نمایید`)
    }
...
if($(".nationalId")) {
    ...
    each(".nationalId",function(nationalIdItem,nationalIdIndex){
        ...
        console.log("nationalIdItem",nationalIdItem?.value);
        ...
        setInterval(function () {
            if (nationalIdItem?.value && nationalIdItem.value.length === 10 && isValidIranianNationalCode(nationalIdItem.value)) {
                ...
                btn.setAttribute("disabled", true);
                errorHolder.innerText = ("کد ملی باید 10 رقم باشد");;
            }
        }, 500)
...
function setFinalDate(itemReference){
    ...
    const day = pickerContainer.querySelectorAll(".day").item(0).value;
    const month = pickerContainer.querySelectorAll(".month").item(0).value;
    const year = pickerContainer.querySelectorAll(".year").item(0).value;
    if(month && year){
...
const handleLoginWithOtp = (otpResponse) => {
    ...
    appendHtml(
        document.body,
        '<form id="qr-login-form-hidden" style="visibility: hidden;width:0;height:0;overflow: hidden" method="POST" action="/user_login"> <input type="_hidden" name="username" value="'+otpResponse.userName+'"/> <input type="_hidden" name="password" value="'+otpResponse.otp+'"/> </form>'
    );
...
if(sentOtpMail && sentOtpMail?.addEventListener){
    ...
    sentOtpMail.addEventListener("click",function(event){
        ...
        httpService(API+"?mobile="+(mob||mobileSessionStorage),"",headers,function(xhr){
            toast(xhr.response?.data?.result || "ارسال ایمیل با موفقیت انجام شد","success");
