...
from .models import Owner, IpcLog, IpcLogHistory, SellModel, Pump, Mojodi, CarInfo, ModemDisconnect, QRScan
from base.models import GsModel, Parametrs, Ticket, Workflow
import datetime
import jdatetime
...
def process_heavy_qr_work(self, owner_id, encrypted_data, st=1, ticket=None, userid=None, lat=None, long=None,
                          failure=None):
    """
    تمام پردازش‌های سنگین QR که قبلاً در تابع encrypt بود به اینجا منتقل می‌شود
    """
    try:
        # کپی منطق پردازش از تابع encrypt به اینجا
        owner = Owner.objects.select_related('role', 'user').only(
            'id', 'qrcode', 'qrcode2', 'user_id', 'role'
        ).get(id=id)
        s = owner.qrcode
        ...
        if rpm_version in parametr.rpm_version:
            if st == 2:
                ...
                result.status_id = 2
                result.save()
                Workflow.objects.create(ticket_id=int(ticket), organization_id=1,
        ...
        return data
            ...
            # اگر خطا داد، ۱ دقیقه بعد دوباره تلاش کن
            self.retry(countdown=60, exc=e)


@shared_task
def bulk_create_sellmodels(sell_data_list):
    """ایجاد دسته‌جمعی SellModel"""
    from .models import SellModel

    objects_to_create = []
    for item in sell_data_list:
        objects_to_create.append(SellModel(**item))

    SellModel.objects.bulk_create(objects_to_create)


@shared_task
def bulk_create_qrtimes(qrtime_data_list):
    """ایجاد دسته‌جمعی QrTime"""
    from .models import QrTime

    objects_to_create = []
    for item in qrtime_data_list:
        objects_to_create.append(QrTime(**item))

    QrTime.objects.bulk_create(objects_to_create)